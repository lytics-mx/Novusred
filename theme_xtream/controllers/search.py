...
class WebsiteSearch(http.Controller):
    ...
    def search_redirect(self, search='', search_type='all', **kw):
        # Reemplazar espacios con guiones en el parámetro de búsqueda
        if search:
        ...
        if search_type == 'brand':
            ...
            product = Product.search([('product_model', '=', search.replace('-', ' '))], limit=1)
            if product:
            ...
            brand = Brand.search([('name', 'ilike', search.replace('-', ' ')), ('active', '=', True)], limit=1)
            if brand:
                return request.redirect('/subcategory?brand_id=%s' % brand.id)
            # Si no es marca, buscar si coincide con una categoría
            Category = request.env['product.category'].sudo()
            category = Category.search([('name', 'ilike', search.replace('-', ' '))], limit=1)
            if category:
